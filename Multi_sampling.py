...
######################################################################################################
Expt_load = 'exp_M_03.hdf5'
Expt_save = 'exp_M_03.hdf5'
LOAD_MODEL = True
No_images_to_add = 3
####################################################################################################3
'''
...
def train_and_evaluate(batch_size=8, img_dir='mess_cb',
	...
	while active_learning_mode:
		model.fit(X_train, Y_train, epochs=epochs, validation_data=(X_val,Y_val),shuffle = True, verbose=1, callbacks=callbacks)
