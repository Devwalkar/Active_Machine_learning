...
######################################################################################################
Expt_load = 'exp_M_BRIEF2.hdf5'
Expt_save = 'exp_M_BRIEF2.hdf5'
LOAD_MODEL = False
No_images_to_add = 5
...
##########################################################################################################3
def calculate_distance(X_train_descriptor):
	global X_oracle_descriptor
...
def create_trainset():
    ...
    Initial_sample = np.random.randint(0,768,1)
    ...
    Y_train = Y_oracle[Selected_Index]

    print 'Creating training dataset:'
    for i in range(No_of_images_to_start_with):
         Dist,Index = calculate_distance(X_train_descriptor)
         Selected_distance = Dist[0]
         Selected_Index = Index[0]
         print 'Adding',str(i),'th image with maximum distance:',str(Selected_distance)
	     Addition = np.reshape(X_oracle[Selected_Index,:,:,:],(1,512,512,3))
	     X_train = np.concatenate((X_train,Addition),axis=0)
	     Y_train = np.append(Y_train,Y_oracle[Selected_Index])
    ...
    return X_train,Y_train,X_train_descriptor
################################################################################################################3
def active_learning():
...
def train_and_evaluate(batch_size=8, img_dir='mess_cb',
	...
	if LOAD_MODEL == True:
		...
		model = classifier.prepare_to_finetune(2e-4)
		model.fit(X_train, Y_train,epochs= epochs,shuffle=True, validation_data=(X_val,Y_val), verbose=1, callbacks=callbacks1)
...
if __name__ == '__main__':
	...
	if LOAD_MODEL==True:
		...
		X,Y,X_oracle,Y_oracle,X_val,Y_val = train.get_data_iterators(batch_size=1,data_dir='mess_cb',target_size=(512, 512),rescale=1/255,fill_mode='constant',load_train_data=True,color_mode='rgb')
    
		X = np.reshape(X,(200,512,512,3))
		X_oracle = np.reshape(X_oracle,(568,512,512,3))
		X_val = np.reshape(X_val,(192,512,512,3))

        X_oracle = np.concatenate((X,X_oracle),axis=0)
        Y_oracle = np.concatenate((Y,Y_oracle),axis=0)

		X_oracle_descriptor = calculate_descriptors(X_oracle)
        X_train,Y_train,X_train_descriptor = create_trainset()
